...
module OmniAuth
  module Strategies
    class Tunnistamo < OpenIDConnect
      ...
      def config
        # Configure the default certificate path for swd, webfinger and
        # openid_connect because of expired Let's Encrypt root certificate.
        # Otherwise these gems would throw a "certificate expired" or "unable to
        # get local issuer certificate" error. If not configured, the HTTPClient
        # (used by SWD, Webfinger and OpenIDConnect) would use its own root CA
        # chain instead as defined here:
        ...
        [::SWD, ::WebFinger, ::OpenIDConnect].each do |dependency|
          dependency.http_config do |http_client|
